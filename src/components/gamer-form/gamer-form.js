...
import { StandartTemplate, defaultCityList } from "../../common/Templates"
...
class GamerForm extends React.PureComponent {
  constructor(props) {
    ...
    // this.state.city = defaultCityList
  ...
  render() {
    ...
    const { setUser, gamers, gamersCount, showResult, startEnterPoints } =
      this.props

    const processUserData = () => {
      let resultKeys = Object.keys(this.state).filter(
        (item) => (item !== "name") & (item !== "city")
      )
      let resultPoints = 0
      resultKeys.map((item) => {
        return (resultPoints = resultPoints + +this.state[item])
      })
